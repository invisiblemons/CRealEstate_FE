...
export class PropertyDetailComponent implements OnInit, OnDestroy {
  ...
  */
  ...
  Fields of routing
  */
  isFromCreateAppm: boolean;
  isFromAppmList: boolean;
    ...
    */
    this.route.queryParams
      .pipe(
        ...
        switchMap((params) => {
          if (params['isFromCreateAppm']) this.isFromCreateAppm = true;
          if (params['isFromAppmList']) this.isFromAppmList = true;
  ...
  hide() {
    ...
    if (this.isFromCreateAppm) {
      this.router.navigate(['/cuoc-hen/tao-moi']);
    } else if (this.isFromAppmList) {
      this.router.navigate(['/cuoc-hen/danh-sach']);
    } else {
      if (this.localStorageServices.getUserObject()) {
        if (this.localStorageServices.getUserObject().status === 2) {
          this.reloadService.routingNotReload('/mat-bang-cho-thue', null);
        } else {
          this.reloadService.routingNotReload('/mat-bang', null);
        }
      } else {
        this.reloadService.routingNotReload('/mat-bang', null);
      }
